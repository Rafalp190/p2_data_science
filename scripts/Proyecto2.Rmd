...
```{r desarmarArmarC1}
edad = 0
completo = subjects[,c(1:5)]
completo$ID = as.factor(completo$ID)
Card1$`Hand grip` = as.numeric(gsub(287, as.numeric(NA), Card1$`Hand grip`) )
for(i in c(8:17)){
  edad = Card1[Card1$edad == i, c(1,9:11)]
  i = as.character(i)
  names(edad) = c("ID",paste("Weight",i),paste("Hand grip",i),paste("Dental",i))
  
  completo = merge(completo, edad, by = "ID", all.x = T)
}

altura17 = Card1[Card1$edad==17,c(1,8)]

completo = merge(completo, altura17, all.x = T )

completo = as.data.frame(completo)

media = 0
columna = 7
for(columna in c(5:36)){
  mediana = completo[,columna]
  mediana = median(as.numeric(mediana), na.rm = T)
  completo[is.na(completo[,columna]),columna]= mediana
}
```
### Subject-Card2
...
g1<- cluster[cluster$grupo==1,]
g2<- cluster[cluster$grupo==2,]
g3<- cluster[cluster$grupo==3,]
g4<- cluster[cluster$grupo==4,]
...
cluster = unique(Card2[,c(3,4,5,7:14)])
cluster$Sex = as.numeric(cluster$Sex)
cluster$`IdSchool 1` = as.numeric(cluster$`IdSchool 1`)
...
plotcluster(cluster[,c(1:11)],km$cluster)
```
...
ggplot(data = cluster, aes(group = grupo, y = `SSF1 mm`, fill = grupo)) + geom_boxplot(outlier.shape = NA) + xlab("Grupos") + ggtitle("SSF1 (mm)") + ylim(c(0,40))
